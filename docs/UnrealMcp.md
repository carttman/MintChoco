...
- A `FunctionInput` added through `add_expression` takes `InputName`, `InputType`
  ...
  re-set. Removing one is the reverse and needs no re-set: drop the pin from the consuming Custom
  node's `Inputs` (an unwired Custom pin fails the compile, so write `Inputs` and `Code` in the
  same call) and then `delete_expression` the `FunctionInput`. `DisplacementScaling` is a plain
