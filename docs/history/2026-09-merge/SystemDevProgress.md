...
`8 → 4 → 0 → 1 → 2 → 7 → 9 → 5 → 6 → 3` — **10단계 전부 코드 완료.**
...
## 3단계: 히어로 랜딩 (2026-09-13)

### 계획서 1번(`UItemAimAbility` 이관)은 하지 않았다

`UItemAimAbility` 의 골격은 “조준 → 확정 → 끝” 이고 `OnItemActivated`·`OnItemEnded` 가
`final` 이라 서브클래스가 흐름을 바꿀 수 없다. 히어로 랜딩은 세 군데가 안 맞는다:

- **확정해도 안 끝난다.** 좌클릭은 내리꽂기의 *시작*이고, 정작 `Burst` 와 스턴은 그 뒤
  **착지할 때** 난다.
- **조준 구간이 어빌리티 전체가 아니다.** 상승 0.6초는 조준이 아니고 호버 2초만 조준이다.
- **조준점을 어빌리티가 안 만든다.** `UUnitMovementComponent::ComputeAimTarget()` 이 만든다 —
  서버가 리플레이에서 같은 값을 내야 해서 거기 있다.

억지로 맞추려면 `UItemAimAbility` 를 고쳐야 하는데 꿀풍선과 디저트 폭격이 이미 그 위에 있다.
끝난 두 아이템을 건드릴 값이 없다고 판단해 히어로 랜딩은 자기 마커·틱 코드를 그대로 뒀다.

### 좌클릭은 RPC 가 아니라 압축 플래그다

이것이 “네트워크 예측이 걸린 유일한 항목” 의 실체다. `UItemSlotComponent::ConfirmAim()` 은
로컬 브로드캐스트 + 서버 RPC 인데, 낙하 판단은 `FSavedMove_Unit` 으로 **리플레이되는**
무브먼트 단계 기계 안에 있다. RPC 로 보내면:
...
스피드 스타가 GAS 속성을 못 쓰는 것과 **같은 함정**이다. 그래서 의도만
**`FLAG_Custom_3`**(마지막 남은 커스텀 플래그)에 실어 보낸다. 네 플래그를 다 썼다:
0 대시 / 1 속도 부스트 / 2 히어로 랜딩 / 3 조기 낙하.

덤으로 `ConfirmAim`·`IsAiming()`·`State.Item.Aiming` 을 **하나도 안 건드렸다.** `IsAiming()` 은
태그 하나만 보는데 히어로 랜딩의 태그는 `State.Item.HeroLanding` 이고, 그 태그는
`IsHeroLandingAllowed()` 가 서버 검증에 쓰고 있어 바꾸면 안 된다. 플래그 경로로 가면
이 충돌이 아예 안 생긴다.

`AUnit::StartFire` 에 4줄을 더했다: 호버 중이면 좌클릭이 `SetWantsHeroDive(true)` 로 간다.
무기는 `State.Item.HeroLanding` 이 `BP_Unit` 의 `TriggerBlockedTags` 에 있어 **이미 막혀 있었다** —
가로채도 없어지는 동작이 없다.

### 충전량

**공중에서 버틴 시간**이다. `HeroPhaseTime / HoverTime` 을 내리꽂기가 시작되는 순간 굳혀
`HeroCharge` 에 둔다. 시간으로만 정해지므로 **서버가 스스로 안다** — 클라이언트가 보내 주는
값이 아니라서 조작할 여지가 없고, 복제할 것도 없다.

효과 배율은 `UHeroLandingProfile::ChargeScaleFor()` 하나에서 나온다
(`Lerp(MinChargeScale, 1, Charge)`, 기본 `MinChargeScale` 0.5). 미리보기 원과 실제 효과가
같은 식을 쓰므로 어긋날 수 없다 — 꿀풍선의 `ComputeThrow` 와 같은 규칙이다.

걸리는 곳: `StunRadius`, `Burst.Speed`(반경이 v² 비례라 √배율), `Burst.Count`.

### 마커: 원 2개 (`ALandingMarker`)

`Items/LandingMarker.h` 를 새로 만들었다. 바깥 원은 최대 반경(`StunRadius`, 지금 600 cm)으로
고정이고, 안쪽 원이 충전량만큼 자라 바깥 원을 향해 간다. 반투명이라 겹친 만큼 진해져
게이지로 읽힌다.

루트가 **스케일 없는 `USceneComponent`** 인 이유: 원 하나를 루트로 삼으면 그 스케일이 다른
원에 곱해져, 안쪽 원을 줄일수록 두께까지 같이 눌린다.

`BP_LandingMarker` 를 **`AStaticMeshActor` 에서 `ALandingMarker` 로 리페어런트**했다.
옛 실린더는 `AStaticMeshActor` 의 **네이티브** 컴포넌트였어서 리페어런트가 알아서 걷어 갔다
(찌꺼기 컴포넌트 없음, 루트는 `Origin`). 메시·재질은 CDO 에 그대로 다시 물렸다
(`/Engine/BasicShapes/Cylinder` + `MI_LandingMarker`). `DA_Item_HeroLanding` 은 손대지 않았다 —
`AimMarkerClass` 가 여전히 `BP_LandingMarker_C` 로 유효하다.

`AimMarkerClass` 의 타입은 `TSubclassOf<AActor>` 로 뒀다. 좁히면 데이터가 어긋났을 때 마커가
통째로 사라지는데, 지금은 `ALandingMarker` 가 아니어도 자리 표시로는 돈다.

### 도포: 한 발에서 산탄 8발로 (데이터만)

처음 데이터는 `Count` 1 · 피치 0~0 이었다. 탄 한 발이 수평으로 나가 1.8 m 앞에 떨어지고
거기 한 덩이를 칠하는 구조라, 충전 배율을 `Count`(1이라 언제나 1)에도 `Speed`(흩어지는
반경이 아니라 그 한 발의 비행 거리)에도 걸 수 없었다. **코드는 그대로 두고 값만 고쳤다.**
...
속도를 **올리지 않고 내린** 이유: 이 탄의 스플랫이 이미 아주 크다.

```
스플랫 반경 = BaseRadius × √SplatVolume + RadiusPerSpeed × 속도
           = 150 × √3 + 0.005 × 310 = 261 cm      (DA_Brush_Bomb, MaxRadius 600 — 안 걸린다)
흩어지는 사거리 = 속도² ÷ (980 × GravityScale) = 310² ÷ 245 = 392 cm
최대 착탄 = 392 × sin(2 × 30°) = 339 cm
칠해지는 반경 = 339 + 261 = 600 cm  ← StunRadius, 마커 바깥 원과 같다
```

꿀벌(산탄 반경 255 · 스플랫 50.6, 비 5:1)을 흉내 내면 안 된다. 히어로 랜딩의 스플랫은
반경 261 이라 같은 비를 맞추면 13 m 까지 번진다. **스플랫이 큰 만큼 산탄은 좁혀야** 마커가
약속한 원 안에 떨어진다.

`MinPitch` 를 0 으로 둔 것이 중요하다. 피치를 20 부터 시작하면(꿀벌의 값) 가장 가까운
착탄이 389 cm 라 **발밑에 도넛 구멍**이 남는다. 0 부터면 낮게 뜬 탄이 발밑을 덮는다.

도포 면적 약 **113 m² = 맵의 1.3 %** (전에는 약 21 m² = 0.25 %). 차지샷(1.8 %) 다음으로 넓다.

### 남은 어긋남: 약한 착지의 도포는 마커보다 크다

충전 배율이 걸리는 곳은 `StunRadius`·`Burst.Speed`(산탄 사거리)·`Burst.Count` 인데,
**스플랫 반경 261 cm 자체는 안 줄어든다**(`SplatVolume` 은 탄 에셋에 있고 스폰마다 못 바꾼다).

| 충전 | 마커·스턴 반경 | 실제 칠해지는 반경 | 탄 수 |
|---|---|---|---|
| 최대 (2초) | 600 cm | 600 cm | 8 |
| 최소 (즉시) | 300 cm | 약 430 cm | 4 |

즉 **일찍 누를수록 표시보다 넓게 칠해진다**(최대 130 cm). 스턴·넉백은 정확히 맞는다.
없애려면 `FPaintBurstParams` 에 도포 배율을 더해야 하는데, 꿀풍선·꿀벌·히어로 랜딩이
같이 쓰는 구조체라 **사전 고지가 필요한 변경**이다. 지금은 두었다.

부수 효과 하나: 탄이 8발이 되면서 한 번의 착지가 나르는 `HitPower` 가 10 → 80 이 된다.
풍선(체력 100)이 한 번에 거의 터진다. 히어로 아이템이라 그럴듯하지만 의도한 값은 아니다.
...
- **3단계는 PIE 에서 검증되지 않았다.** 특히 **리슨 서버 + 클라이언트 2인**에서 조기 낙하가
  양쪽에 같게 보이는지, 고무줄이 없는지는 눈으로 봐야 한다. 압축 플래그를 썼으니 구조상
  맞지만, 예측은 실제로 붙여 보기 전에는 장담할 수 없다.
- 마커 두 원의 크기(바깥 600 cm)는 계산으로만 잡았다. 지름 12 m 라 화면에서 클 수 있다.
...
- 테스트: `UnrealEditor-Cmd.exe <uproject> -ExecCmds="Automation RunTests MintChoco; Quit" -unattended -nullrhi -abslog=D:\git\MintChoco\Saved\Logs\AutoTest.log`
  로그에서 `Result={Fail}` 을 찾는다. 현재 기준선은 **42건 전부 통과**다.
