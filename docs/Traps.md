...
| Symptom | Check first |
|---|---|
| A Niagara effect stays its asset colour instead of the team colour, or flashes the asset colour for one frame | Two halves have to meet. The **asset** must declare a `User.TintColor` LinearColor (`TeamLook::NiagaraTintParameter`) and actually drive its look from it — usually by linking `InitializeParticle` -> `Color`, which reaches the material through the renderer's `ColorBinding` (`Particles.Color`) and a `Particle Color` node in the material; a system without that variable silently ignores the write, which is what makes it look like the code is broken. The **spawner** must write `TeamLook::GetColor(PaintId)` into it, and must do so **before the component activates**: a user parameter set on an already-active component is deferred to the next tick (`SetVariable_Deferred`), so an effect that emits everything on its first tick is born in the asset colour and only corrects a frame later. Spawn with `bAutoActivate=false`, set, then `Activate()`. Writers: `APaintBurst::BeginPlay`, `UItemSlotComponent` (ActivateFX), `AUnit::UpdateHeldItemFX` (HeldFX), `UPaintWeaponComponent::TintTeamFX`, `UPaintballProfile`, `APaintRain`, `UGA_DessertBombardment`. |
...
| The sweet spinner should spin more (or `Turns` changes nothing) | The spray direction comes from the **hand socket of `SpinAnimation`** (`UGA_SweetSpinner::ComputeHandMuzzle`), so the number of turns is whatever the clip turns, once per loop. `USweetSpinnerProfile::Turns` is only the fallback used when the socket is missing — editing it on a profile whose `HandSocket` resolves does nothing at all. The real knob is `SpinLoops`: the spin phase is `clip length × SpinLoops` (`SweetSpinner::ComposeSpinPhase`), the pose slot loops the clip in the AnimGraph, and the `Spinner Volley Window` notify repeats with it — `UGA_SweetSpinner::IsInVolleyWindow` skips the shots that fall outside the window of the loop they land in, off one timer so the rounding in the shot count cannot accumulate across loops. `Duration` has to have room for `start + clip × SpinLoops + end` or the spin is cut where the tag drops; `LogUnsetReferences` warns when it does not. Tests: `MintChoco.Items.SweetSpinner.SpinLoops`, `.Phases`, `.Recovery`. |
...
| A weapon hit should stun, or stuns the wrong length | `FPaintDeposit::StunDuration` / `StunSuperArmorDuration` on the contact (paintball `Deposit`, sniper `Impact`): `ApplyHit` and the sniper's pawn branch call `StrikeUnit`, which skips same-colour units and calls `AUnit::TryApplyStun(Stun, SuperArmor)`. A Charged profile scales the stun by the release charge (`FPaintFireContext::ChargeFraction`, sent in `ServerFire` as a byte); `MinChargeToFire` below 1 lets a partial charge fire. Items keep `UItemSettings` (2 s / 4 s) through the parameterless `TryApplyStun()`. The stun lives on the **paintball** asset, so swapping a weapon's `Paintball` reference drops it: the shotgun (`BP_Unit` → `_Test/DA_Weapon_Fan_T`) fires `_Test/DA_Paintball_Heavy_Trail_T` — the **`_Test` copies are the live ones**, see the row below. `DA_Paintball_HeavyStun` (0.25) and the non-`_T` originals are what it used to fire and nothing references them. That ball's `Deposit.StunDuration` is **0** on purpose: the left-click gun paints and pushes a balloon (`HitPower` 25) but never stuns anyone, so the charged sniper's `Impact` is the only weapon stun left in the game. A left click that stuns again means someone put a number back on that one field. |
| A weapon asset is edited and nothing changes in PIE | `BP_Unit`'s two weapon components point at the **`Content/Blueprints/Weapons/_Test/` copies**, not the originals beside them: `PaintWeapon` → `DA_Weapon_Fan_T` → `DA_Paintball_Heavy_Trail_T` → `DA_Brush_Mop_T` / `DA_Brush_Trail_Fan_T` / `DA_Brush_Feet_T`, `SecondaryWeapon` → `DA_Weapon_Sniper_T` → `DA_Paintball_SniperVolley_T`. The `_T` chain is self-contained except for `DA_Scatter_Fan` and `DA_Splash_Paintball`, which both chains share — so a scatter edit **does** land and a ball or brush edit does not, which is what makes this so convincing. The non-`_T` originals are referenced by nothing. Read the component's `Profile` on the BP before editing any weapon asset, and grep with the trailing `_T` in the pattern (`DA_Weapon_[A-Za-z_]+`): a character class without the underscore silently truncates the name to the original's. |
...
| Holding the trigger fires one shot, or the infinite-ammo item looks like it turns the gun automatic | Repeating while held is `FireMode` on the profile, and the item never touches it: `UPaintWeaponComponent::IsShotFree()` waives the ink cost and clamps the cadence, nothing else (`GetEffectiveShotInterval` = min(profile, `FreeShotInterval` 0.1), `GetEffectiveChargeTime` = min(profile, `FreeShotChargeTime` 0.5)). In `Single` the interval is only a floor **between two presses**, so a held button fires once and clicks faster than the floor are silently swallowed — shortening that floor is what makes the item feel like rapid fire, and it was the real difference, not repeating. The left-click gun (`_Test/DA_Weapon_Fan_T`) is `Automatic` at 10 Hz so holding matches the item exactly. `Continuous` is not an option for a gun: it spawns a ball per frame, and `MintChoco.Paint.Weapons.ProfileAssets` fails it. |
| Ink refills faster (or not) than the number on the tank says | Every rate goes through `UInkTankComponent::GetRefillPerSecond()`: `RefillPerSecond` normally, times `DashRefillMultiplier` while the board is out (a value under 1 is read as 1, so the board can never *slow* the refill). The tank does not look the unit up — `AUnit::HandleDashStateChanged` pushes it in with `SetDashing`, and that notification only reaches the machines whose dash flag actually moved (the owner and the server), which is also the only place the refill runs (`TickComponent` guards on `ROLE_Authority`). Nothing about this is replicated on its own; the server's `Ink` value is. The board does **not** shorten `RefillDelayAfterSpend`, so firing and immediately dashing still costs the pause. Test: `MintChoco.Ink.Tank.DashRefill`. |
| An item never drops although its asset, ability and icon all exist | The drop table is the `+Items` list in `[/Script/MintChoco.ItemSettings]` (`Config/DefaultGame.ini`), not the folder: the game mode spawns from it and `FindItemByStateTag` maps a replicated tag back through it. The chocolate fountain is commented out there and is the reason to look at the file rather than the code. Nothing warns — a missing entry just never comes up in the roll. |
