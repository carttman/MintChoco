...
| The board does not lean on left/right, leans the wrong way, or leans only on one machine | `UUnitAnimInstance` sets `BoardLean` while `bDashAnimationActive`: `FUnitAnimMath::BoardLeanInput` projects the movement acceleration's **direction** on the body's right axis, times `BoardLeanMaxDegrees` (15, flip the sign to flip the lean), eased by `BoardLeanInterpSpeed`. Direction only, because simulated proxies get no replicated acceleration and the engine fills it with the velocity's unit vector. `AUnit::SetMeshLean` rolls the mesh about the capsule's forward axis on top of the rest rotation captured once, and writes the same rotation through `CacheInitialMeshOffset`: network smoothing rewrites the mesh's relative rotation from `GetBaseRotationOffset` every tick, so a plain `SetRelativeRotation` would be erased on other clients. Tests: `MintChoco.Anim.BoardLean.*`. |
