...
namespace
{
	...
	/** Handlers the warmup allocates up front. A volley of pellets books one each, so a shotgun's worth. */
	constexpr int32 PrewarmHandlers = 32;
}
...
void UPaintSplashSubsystem::Prewarm()
{
	if (!DrawsMarks())
	{
		return;
	...
	while (Handlers.Num() < PrewarmHandlers)
	{
		Handlers.Add(NewObject<UPaintSplashLandingHandler>(this));
	}
	UE_LOG(LogPaint, Log, TEXT("splash prewarm: %d landing handlers ready."), Handlers.Num());
}
