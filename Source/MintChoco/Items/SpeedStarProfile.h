...
public:
	...
	/**
	 * 자국을 찍을 바닥을 발밑에서 이만큼까지 찾는다(cm).
	 *
	 * 짧으면 땅에 붙어 달릴 때만 칠해진다. 점프나 히어로 랜딩처럼 떠 있는 동안에도 지나간 자리가
	 * 칠해지려면 넉넉해야 한다. 트레이스는 가장 가까운 바닥을 맞히므로, 늘려도 땅에 붙어 있을 때의
	 * 결과는 달라지지 않는다. 다만 이 거리가 곧 "얼마나 높은 데서 칠할 수 있는가"이기도 하다.
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "SpeedStar", meta = (ClampMin = "0", ForceUnits = "cm"))
	float MarkGroundReach = 1200.0f;
